...
country <- fromJSON(content(response, "text"))
filter(country, name_en == "Syrian Arab Republic") %>%
  select(country_code)
...
refugees <- select(refugees, year, usa = value)
...
response <- GET("http://data.unhcr.org/api/stats/time_series_all_years.json?", query = query.params)
other <- fromJSON(content(response,"text"))
other <- select(other, year, other = value)

left_join(refugees, other, by="year") %>%
